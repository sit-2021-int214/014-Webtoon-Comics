...
```
cartoons <- webToon %>% filter(webToon$Update == "COMPLETED", webToon$Rating>9.50, na.rm = TRUE)
cartoons %>% summarise(Name,Genre,Rating) #กำหนดให้แสดงผลลัพธ์แค่ชื่อ หมวดหมู่ และ rating ของการ์ตุน
```

Result

```           
        cartoons$Name cartoons$Genre cartoons$Rating
